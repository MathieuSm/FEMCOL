...
def PlotRegressionResults(Model, Data, Alpha=0.95):
    ...
    if float(p) <= 0.05:
        ...
        Axes.scatter(X_np[:, 1][Data['Gender'] == 'M'], Y_Obs_np[Data['Gender'] == 'M'],
                     c=list(tuple(male_age.tolist())), cmap='plasma_r', vmin=Data['Age'].min(), vmax=Data['Age'].max(),
                     label='male', marker='s')
        ...
        # test_data_df = pd.DataFrame(test_data, columns=['BMC', 'MW'])
        # plt.plot(test_data_df['BMC'], test_data_df['MW'], linestyle='none', color='b', marker='x')
        # plt.colorbar()
        # plt.show()
    else:
